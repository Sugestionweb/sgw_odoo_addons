...
class SupportTicketController(http.Controller):
    ...
    def support_approve(self, ticket_id, **kwargs):
        support_ticket = (
            request.env["website.support.ticket"].sudo().browse(int(ticket_id))
        )

        awaiting_approval = request.env["ir.model.data"].get_object(
        ...
        if support_ticket.approval_id.id == awaiting_approval.id:
            ...
            website_ticket_state_approval_accepted = request.env[
                "ir.model.data"
            ].get_object("website_support", "website_ticket_state_approval_accepted")
            support_ticket.state_id = website_ticket_state_approval_accepted.id

            # Also change the approval
            approval_accepted = request.env["ir.model.data"].get_object(
                "website_support", "approval_accepted"
            )
            ...
            notification_template = (
                ...
                .sudo()
                .get_object("website_support", "support_ticket_approval_user")
            )
            support_ticket_menu = (
                request.env["ir.model.data"]
                .sudo()
                .get_object("website_support", "website_support_ticket_menu")
            )
            support_ticket_action = (
                request.env["ir.model.data"]
                .sudo()
                .get_object("website_support", "website_support_ticket_action")
            )
            ...
            for my_user in support_ticket.category_id.cat_user_ids:
                ...
                values["body_html"] = (
                    ...
                    .replace(
                        ...
                        + str(support_ticket_menu.id)
                        + "&action="
                        + str(support_ticket_action.id),
                    )
                    .replace("_user_name_", my_user.partner_id.name)
                )
    ...
    def support_disapprove(self, ticket_id, **kwargs):
        ...
        if support_ticket.approval_id.id == awaiting_approval.id:
            ...
            website_ticket_state_approval_rejected = request.env[
                "ir.model.data"
            ].get_object("website_support", "website_ticket_state_approval_rejected")
            support_ticket.state_id = website_ticket_state_approval_rejected.id

            # Also change the approval
            approval_rejected = request.env["ir.model.data"].get_object(
                "website_support", "approval_rejected"
            )
    ...
    @http.route(
        "/support/subcategories/field/fetch", type="http", auth="public", website=True
    )
    def support_subcategories_field_fetch(self, **kwargs):
        ...
        if values["subcategory"] != "undefined":
            sub_category_fields = (
                request.env["website.support.ticket.subcategory.field"]
                .sudo()
                .search([("wsts_id", "=", int(values["subcategory"]))])
            )
        else:
            return ""
        ...
        if sub_category_fields:
            for sub_category_field in sub_category_fields:
                ...
                return_string += (
                    '  <label class="control-label" for="efield_'
                    + str(sub_category_field.id)
                    + '">'
                ...
                if sub_category_field.type == "textbox":
                    return_string += (
                        '  <input type="text" required="required" class="form-control" name="efield_'
                        + str(sub_category_field.id)
                        + '">\n'
                    )
                elif sub_category_field.type == "dropbox":
                    return_string += (
                        '  <select required="required" class="form-control" name="efield_'
                        + str(sub_category_field.id)
                        + '">\n'
                    )
    ...
    @http.route(
        "/support/subcategories/fetch", type="http", auth="public", website=True
    )
    def support_subcategories_fetch(self, **kwargs):
        ...
        sub_categories = (
            request.env["website.support.ticket.subcategory"]
            .sudo()
            .search([("parent_category_id", "=", int(values["category"]))])
        )
    ...
    @http.route(
        "/support/survey/<portal_key>", type="http", auth="public", website=True
    )
    def support_ticket_survey(self, portal_key):
        """Display the survey"""

        support_ticket = (
            request.env["website.support.ticket"]
            .sudo()
            .search([("portal_access_key", "=", portal_key)])
        )
        ...
        if support_ticket.support_rating:
            ...
            return http.request.render(
                "website_support.support_ticket_survey_page",
                {"support_ticket": support_ticket},
            )

    @http.route(
        "/support/survey/process/<portal_key>", type="http", auth="public", website=True
    )
    ...
    def support_account_create(self, **kw):
        ...
        setting_allow_user_signup = request.env["ir.default"].get(
            "website.support.settings", "allow_user_signup"
        )
    ...
    @http.route(
        "/support/account/create/process", type="http", auth="public", website=True
    )
    def support_account_create_process(self, **kw):
        ...
        if setting_allow_user_signup:
            ...
            support_group = (
                request.env["ir.model.data"]
                .sudo()
                .get_object("website_support", "support_group")
            )
            support_group.users = [(4, new_user.id)]

            # Also add them to the portal group so they can access the website
            group_portal = (
                request.env["ir.model.data"].sudo().get_object("base", "group_portal")
            )
            ...
            request.session.authenticate(
                request.env.cr.dbname, values["login"], values["password"]
            )
    ...
    def support_help(self, **kw):
        ...
        help_groups = (
            http.request.env["website.support.help.group"]
            .sudo()
            .search(
                [
                    "|",
                    ("group_ids", "=", False),
                    ("group_ids", "in", permission_list),
        ...
        manager = False
        if (
            request.env["website.support.department.contact"]
            .sudo()
        ...
        return http.request.render(
            "website_support.support_help_pages",
            {
                "help_groups": help_groups,
                "setting_allow_user_signup": setting_allow_user_signup,
                "manager": manager,
            },
        )
    ...
    def support_ticket_reporting(self, **kw):
        ...
        department = (
            request.env["website.support.department.contact"]
            .search([("user_id", "=", request.env.user.id)])[0]
            .wsd_id
        )
        ...
        return http.request.render(
            "website_support.support_ticket_reporting",
    ...
    def support_submit_ticket(self, **kw):
        ...
        ticket_categories = (
            http.request.env["website.support.ticket.category"]
            .sudo()
            .search(
                [
                    "|",
                    ("access_group_ids", "in", category_access),
        ...
        setting_allow_website_priority_set = request.env["ir.default"].get(
            "website.support.settings", "allow_website_priority_set"
        )

        return http.request.render(
            "website_support.support_submit_ticket",
            {
                "categories": ticket_categories,
                "priorities": http.request.env["website.support.ticket.priority"]
                .sudo()
                .search([]),
    ...
    @http.route(
        "/support/feedback/process/<help_page>",
        type="http",
        auth="public",
        website=True,
    )
    ...
    def help_page_create(self, group_id, help_page_name, **post):
        ...
        help_page = request.env["website.support.help.page"].create(
            {"group_id": group_id, "name": help_page_name}
        )
    ...
    @http.route(
        '/support/help/<model("website.support.help.group"):help_group>',
        type="http",
        auth="public",
        website=True,
    )
    def help_group(self, help_group):
        ...
        if help_group.website_published:
            return http.request.render(
                "website_support.help_group", {"help_group": help_group}
            )
        else:
    ...
    @http.route(
        [
            """/support/help/<model("website.support.help.group"):help_group>/<model("website.support.help.page", "[('group_id','=',help_group[0])]"):help_page>"""
        ],
        type="http",
        auth="public",
        website=True,
    ...
    def help_page(self, help_group, help_page, enable_editor=None, **post):
        ...
        if (
            help_group.website_published
            and help_page.website_published
            and (
                request.env.user in help_group.sudo().group_ids.users
                or len(help_group.group_ids) == 0
            )
        ):
            return http.request.render(
                "website_support.help_page", {"help_page": help_page}
            )
        else:
            return request.render("website.404")

    @http.route(
        "/support/ticket/process", type="http", auth="public", website=True, csrf=True
    )
    def support_process_ticket(self, **kwargs):
        ...
        if setting_google_recaptcha_active:
            ...
            payload = {
                "secret": setting_google_captcha_secret_key,
        ...
        if http.request.env.user.name != "Public user":
            ...
            if "priority" in values and (
                setting_allow_website_priority_set == "partner"
                or setting_allow_website_priority_set == "everyone"
            ):
                create_dict["priority_id"] = int(values["priority"])

            # Add to the communication history of the logged in user
            partner.message_post(
                body="Customer " + partner.name + " has sent in a new support ticket",
            ...
            if (
                "priority" in values
                and setting_allow_website_priority_set == "everyone"
            ):
                create_dict["priority_id"] = int(values["priority"])
    ...
    def support_ticket_view(self, ticket):
        ...
        return http.request.render(
            "website_support.support_ticket_view",
            {
                "support_ticket": support_ticket,
                "setting_max_ticket_attachments": setting_max_ticket_attachments,
                "setting_max_ticket_attachment_filesize": setting_max_ticket_attachment_filesize,
    ...
    @http.route(
        "/support/portal/ticket/comment", type="http", auth="public", website=True
    )
    def support_portal_ticket_comment(self, **kw):
        ...
        support_ticket = (
            http.request.env["website.support.ticket"]
            .sudo()
            .search([("portal_access_key", "=", values["portal_access_key"])])[0]
        )

        request.env["website.support.ticket.message"].sudo().create(
            {
        ...
        support_ticket.state_id = (
            request.env["ir.model.data"]
            .sudo()
            .get_object("website_support", "website_ticket_state_customer_replied")
        )
        ...
        request.env["website.support.ticket"].sudo().browse(
            support_ticket.id
        ).message_post(
            body=values["comment"],
    ...
    def support_ticket_comment(self, **kw):
        ...
        if ticket.partner_id.id == http.request.env.user.partner_id.id:
            ...
            message_post = ticket.sudo().message_post(
                body=values["comment"],
                subject="Support Ticket Reply",
                message_type="comment",
                attachments=attachments,
    ...
    def support_ticket_close(self, **kw):
        ...
        if (
            ticket.partner_id.id == http.request.env.user.partner_id.id
            or ticket.partner_id in http.request.env.user.partner_id.stp_ids
        ):

            customer_closed_state = (
                request.env["ir.model.data"]
                .sudo()
                .get_object("website_support", "website_ticket_state_customer_closed")
            )
    ...
    def support_help_autocomplete(self, **kw):
        ...
        help_pages = (
            request.env["website.support.help.page"]
            .sudo()
            .search([("name", "=ilike", "%" + values["term"] + "%")], limit=5)
        )

        for help_page in help_pages:
            return_item = {
                "label": help_page.name,
                "value": "/support/help/"
                + slug(help_page.group_id)
                + "/"
                + slug(help_page),
            }
