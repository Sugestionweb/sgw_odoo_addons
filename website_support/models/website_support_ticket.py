...
class WebsiteSupportTicket(models.Model):
    ...
    def _read_group_state(self, states, domain, order):
        ...
        exclude_states = [
            staff_replied_state.id,
            customer_replied_state.id,
            customer_closed.id,
            staff_closed.id,
        ]
    ...
    parent_company_id = fields.Many2one(
        string="Parent Company", related="partner_id.company_id"
    )
    ...
    category_id = fields.Many2one(
        "website.support.ticket.category",
        string="Category",
        track_visibility="onchange",
    )
    sub_category_id = fields.Many2one(
        "website.support.ticket.subcategory", string="Sub Category"
    )
    subject = fields.Char(string="Subject")
    description = fields.Text(string="Description")
    state_id = fields.Many2one(
        "website.support.ticket.state",
        group_expand="_read_group_state",
        default=_default_state,
        string="State",
    )
    conversation_history_ids = fields.One2many(
        "website.support.ticket.message", "ticket_id", string="Conversation History"
    )
    ...
    extra_field_ids = fields.One2many(
        "website.support.ticket.field", "wst_id", string="Extra Details"
    )
    planned_time = fields.Datetime(string="Planned Time")
    planned_time_format = fields.Char(
        string="Planned Time Format", compute="_compute_planned_time_format"
    )
    approval_id = fields.Many2one(
        "website.support.ticket.approval",
    ...
    disapprove_url = fields.Char(
        compute="_compute_disapprove_url", string="Disapprove URL"
    )
    ...
    sla_timer_format = fields.Char(
        string="SLA Timer Format", compute="_compute_sla_timer_format"
    )
    ...
    def _compute_sla_timer_format(self):
        ...
        self.sla_timer_format = "{:02.0f}:{:02.0f}".format(
    ...
    def _compute_planned_time_format(self):
        ...
        if self.planned_time and self.partner_id and self.partner_id.lang:
            ...
            if self.partner_id.tz:
                ...
                self.planned_time_format = (
                    local_time.strftime(
                        partner_language.date_format
                        + " "
                        + partner_language.time_format
                    )
                    + " "
                    + self.partner_id.tz
                )
    ...
    def message_new(self, msg, custom_values=None):
        ...
        search_partner = (
            self.env["res.partner"].sudo().search([("email", "=", from_email)])
        )
        ...
        return super(WebsiteSupportTicket, self).message_new(
    ...
    def message_update(self, msg_dict, update_vals=None):
        ...
        self.conversation_history_ids.create(
            {"ticket_id": self.id, "by": "customer", "content": body_short}
        )

        return super(WebsiteSupportTicket, self).message_update(
            msg_dict, update_vals=update_vals
        )
    ...
    def _needaction_domain_get(self):
        ...
        return [
            "|",
            ("state", "=", open_state.id),
            ("state_id", "=", custom_replied_state.id),
        ]
    ...
    def create(self, vals):
        ...
        if new_id.partner_id.sla_id:
            ...
            for sla_rule in new_id.partner_id.sla_id.rule_ids:
                ...
                for sla_rule_con in sla_rule.condition_ids:
                    if (
                        sla_rule_con.type == "category"
                        and new_id.category_id.id != sla_rule_con.category_id.id
                    ):
                        all_true = False
                    elif (
                        sla_rule_con.type == "subcategory"
                        and new_id.sub_category_id.id != sla_rule_con.subcategory_id.id
                    ):
                        all_true = False
                    elif (
                        sla_rule_con.type == "priority"
                        and new_id.priority_id.id != sla_rule_con.priority_id.id
                    ):
                        all_true = False
        ...
        ticket_open_email_template = (
            self.env["ir.model.data"]
            .get_object("website_support", "website_ticket_state_open")
            .mail_template_id
        )
        ...
        notification_template = (
            self.env["ir.model.data"]
            .sudo()
            .get_object("website_support", "new_support_ticket_category")
        )
        support_ticket_menu = (
            self.env["ir.model.data"]
            .sudo()
    ...
    def write(self, values, context=None):
        ...
        if "category_id" in values:
            change_category_email = (
                self.env["ir.model.data"]
                .sudo()
                .get_object("website_support", "new_support_ticket_category_change")
            )
    ...
    def send_survey(self):
        notification_template = (
            self.env["ir.model.data"]
            .sudo()
            .get_object("website_support", "support_ticket_survey")
        )
...
class WebsiteSupportTicketMessage(models.Model):
    ...
    def create(self, values):
        ...
        if values["by"] == "customer":
            customer_reply_email_template = self.env["ir.model.data"].get_object(
                "website_support", "support_ticket_customer_reply_wrapper"
            )
...
class WebsiteSupportTicketSubCategory(models.Model):
    ...
    parent_category_id = fields.Many2one(
        "website.support.ticket.category", required=True, string="Parent Category"
    )
    additional_field_ids = fields.One2many(
        "website.support.ticket.subcategory.field",
        "wsts_id",
...
class WebsiteSupportTicketSubCategoryField(models.Model):
    ...
    wsts_id = fields.Many2one(
        "website.support.ticket.subcategory", string="Sub Category"
    )
    name = fields.Char(string="Label", required="True")
    type = fields.Selection(
        [("textbox", "Textbox"), ("dropbox", "Dropbox")],
        default="textbox",
        required="True",
        string="Type",
    )
    dropbox_type = fields.Selection(
        [("static", "Static")], default="static", string="Dropbox Type"
    )
    value_ids = fields.One2many(
        "website.support.ticket.subcategory.field.value", "wstsf_id", string="Values"
    )
...
class WebsiteSupportTicketSubCategoryFieldValue(models.Model):
    ...
    wstsf_id = fields.Many2one(
        "website.support.ticket.subcategory.field", string="Subcategory Field"
    )
...
class WebsiteSupportTicketState(models.Model):
    ...
    mail_template_id = fields.Many2one(
        "mail.template",
        domain="[('model_id','=','website.support.ticket')]",
        string="Mail Template",
        help="The mail message that the customer gets when the state changes",
    )
    unattended = fields.Boolean(
        string="Unattended",
        help="If ticked, tickets in this state will appear by default",
    )
...
class WebsiteSupportTicketUsers(models.Model):
    ...
    cat_user_ids = fields.Many2many(
        "website.support.ticket.category", string="Category Users"
    )
...
class WebsiteSupportTicketClose(models.TransientModel):
    ...
    def _onchange_template_id(self):
        if self.template_id:
            values = self.env["mail.compose.message"].generate_email_for_composer(
                self.template_id.id, [self.ticket_id.id]
            )[self.ticket_id.id]
    ...
    def close_ticket(self):
        ...
        closed_state = (
            self.env["ir.model.data"]
            .sudo()
            .get_object("website_support", "website_ticket_state_staff_closed")
        )
        ...
        setting_auto_send_survey = self.env["ir.default"].get(
