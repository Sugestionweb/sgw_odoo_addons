...
class WebsiteSupportSLA(models.Model):
    ...
    rule_ids = fields.One2many(
        "website.support.sla.rule",
        "vsa_id",
        string="Rules",
        help="If a ticket matches mutiple rules then the one with the lowest response time is used",
    )
    alert_ids = fields.One2many(
        "website.support.sla.alert", "vsa_id", string="Email Alerts"
    )
...
class WebsiteSupportSLARule(models.Model):
    ...
    condition_ids = fields.One2many(
        "website.support.sla.rule.condition",
        "wssr_id",
        string="Conditions",
        help="All conditions have to be fulfilled for the rule to apply, e.g. priority='High' AND category='Tech Support'",
        required="True",
    )
    response_time = fields.Float(
        string="Response Time",
        required="True",
        help="If the support ticket matches the conditions then it has to be completed within this amount of time, e.g. high priority tech support ticket within 1 hour",
    )
    countdown_condition = fields.Selection(
        [("business_only", "Business Only"), ("24_hour", "24 Hours")],
        default="24_hour",
        required="True",
        help="During what time do we start counting down the SLA timer",
    )
...
class WebsiteSupportSLARuleCondition(models.Model):
    ...
    display_value = fields.Char(
        string="Display Value", compute="_compute_display_value"
    )
...
class WebsiteSupportSLAAlert(models.Model):
    ...
    vsa_id = fields.Many2one("website.support.sla", string="SLA")
    alert_time = fields.Float(
        string="Alert Time",
        help="Number of hours before or after SLA expiry to send alert",
    )
