...
class WebsiteSupportHelpGroup(models.Model):
    ...
    def _page_count(self):
        ...
        self.page_count = self.env["website.support.help.page"].search_count(
            [("group_id", "=", self.id)]
        )
...
class WebsiteSupportHelpPage(models.Model):
    ...
    feedback_ids = fields.One2many(
        "website.support.help.page.feedback", "hp_id", string="Feedback"
    )
    feedback_average = fields.Float(
        string="Feedback Average Rating", compute="_compute_feedback_average"
    )
